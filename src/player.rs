...
impl Player {
    pub fn new(pos: Vec2f) -> Player {
         ...
         move_dir_buf: Vec::new(), rotate_dir_buf: Vec::new() }
    }
    ...
    fn add_dir(&mut self, use_move_dir_buf: bool, dir: Direction) {
        let buf = if use_move_dir_buf { &mut self.move_dir_buf } else { &mut self.rotate_dir_buf };
    ...
    fn handle_event(&mut self, event: Event) -> Option<Event> {
        match event {
            Event::KeyDown{keycode: Some(keycode), ..} => {
                match keycode {
                    Keycode::W => { self.add_dir(true, Direction::Forward); None }
                    Keycode::S => { self.add_dir(true, Direction::Backward); None }
                    Keycode::Q => { self.add_dir(true, Direction::Left); None }
                    Keycode::E => { self.add_dir(true, Direction::Right); None }

                    Keycode::A => { self.add_dir(false, Direction::Left); None }
                    Keycode::D => { self.add_dir(false, Direction::Right); None }
                    _ => Some(event)
                }
            },
            Event::KeyUp{keycode: Some(keycode), ..} => {
                match keycode {
                    Keycode::W => { self.remove_dir(true, Direction::Forward); None }
                    Keycode::S => { self.remove_dir(true, Direction::Backward); None }
                    Keycode::Q => { self.remove_dir(true, Direction::Left); None }
                    Keycode::E => { self.remove_dir(true, Direction::Right); None }

                    Keycode::A => { self.remove_dir(false, Direction::Left); None }
                    Keycode::D => { self.remove_dir(false, Direction::Right); None }
                    _ => Some(event)
                }
            },
            _ => Some(event)
