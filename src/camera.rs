...
impl GameComponent for Camera {
    fn draw(&mut self, _: &Game, _: u32) -> Option<&Surface> {
        if self.last_position_drawn_from == *self.player.borrow().pos() &&
           self.last_dir_drawn_from == *self.player.borrow().dir() {
            return Some(&self.camera_view);
        }

        self.last_position_drawn_from = self.player.borrow().pos().clone();
        self.last_dir_drawn_from = self.player.borrow().dir().clone();

        for (ray_dir, column_index) in Rays::new(self) {
            ...
            let org_pos = self.player.borrow().pos().clone();
            let mut pos = self.player.borrow().pos().clone();
            ...
            loop {
                ...
                let ray_continuation_coeffs = Vec2f::new(dst_to_wall.x() / ray_dir.x(),
                    dst_to_wall.y() / ray_dir.y());

                let ray_continuation_coeff = if 
                    ray_continuation_coeffs.x().abs() < ray_continuation_coeffs.y().abs()
                    { ray_continuation_coeffs.x().abs() } else { ray_continuation_coeffs.y().abs() };
...
impl Rays {
    fn new(camera: &Camera) -> Rays {
        ...
        let horizontal_offset_per_column = camera_plane * 2.0 / f32::from(camera.screen_width.clone() as u16);
